...
class CSPStage(nn.Layer):
    def __init__(self,
        ...
        self.conv1 = ConvBNLayer(ch_out, ch_out//2, 1, 1, act=act)
        self.conv2 = ConvBNLayer(ch_out, ch_out//2, 1, 1, act=act)
        ...
        if attn:
            ...
            self.attn = None
        self.conv3 = ConvBNLayer(ch_out, ch_out, 1, 1, act=act)
...
class CSPConvNext(nn.Layer):
    def __init__(
        ...
        self.stages = nn.Sequential(*[(str(i), CSPStage(
            Block, dims[i], dims[i+1], depths[i], stride[i], dp_rates[sum(depths[:i]) : sum(depths[:i+1])],act=nn.Silu))
                                      for i in range(n)])
...
def CSPConvNext_tiny_v2(pretrained=False, use_ssld=False, **kwargs):
    model = Model( **kwargs)
    _load_pretrained(
...
if __name__=="__main__":
     model  = CSPConvNext_tiny_v2()
     # Total Flops: 2695941504     Total Params: 19758056
